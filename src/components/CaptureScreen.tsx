  ...
  const handleProcessAI = async () => {
    ...
    try {
      ...
      if (data.tasks && Array.isArray(data.tasks)) {
        ...
        setText("");
        setStatusMessage(`Успішно розпарсено ${data.tasks.length} задач(-і) у Завдання!`);
        setTimeout(() => {
          onNavigateToInbox();
        }, 600);
      } else {
    ...
    <div className="space-y-6 pb-24 max-w-2xl mx-auto">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-2 shadow-sm">
        <div className="flex items-center gap-2 text-indigo-400 font-semibold text-sm">
          ...
          <span>Крок 1: Потік (Захоплення думок)</span>
        </div>
        <h2 className="text-xl font-bold text-white tracking-tight leading-snug">
          Справи чекають! Що вам треба зробити сьогодні чи в найближчі дні?
        </h2>
        <p className="text-sm text-slate-300 leading-relaxed">
          Покажіть нам, що таке справжній потік думок! Диктуйте й пишіть все, що в голові, а AI сам зловить все важливе та організує ваш розклад.
        </p>
      </div>
